...
export function AdminSidebar({
    ...
    return (
        <TooltipProvider>
            ...
            bg-admin-sidebar text-admin-sidebar-fg">
                <div
                    className={`flex items-center gap-2 pt-6 pb-4 ${collapsed ? 
                        "justify-center px-2" : "px-6"}`}
                >
                    <div className="min-w-0 flex-1">
                        {collapsed ? (
                            <div
                                className="grid size-9 place-items-center rounded-lg
                                bg-white/10 font-display text-base text-white">
                                V
                            </div>
                        ) : (
                            <>
                    ...
                    {onToggleCollapsed ? (
                        <button
                            ...
                            className="hidden size-8 shrink-0 place-items-center rounded-md
                            text-admin-sidebar-fg/70 hover:bg-white/10 hover:text-white lg:grid"
                ...
                <div className={`pt-4 pb-3 ${collapsed ? "px-2" : "px-4"}`}>
                    {collapsed ? (
                        <Tooltip delayDuration={150}>
                            <TooltipTrigger asChild>
                                <Link
                                    ...
                                    bg-admin-sage text-admin-ink hover:bg-admin-sage-deep"
                                >
                        ...
                        <Link
                            ...
                            bg-admin-sage px-4 py-2.5 text-sm font-medium text-admin-ink
                            hover:bg-admin-sage-deep"
                        >
                            <Plus className="size-4"/>
                            New Appointment
                        </Link>
                ...
                <div className={`border-t border-white/10 py-3 ${collapsed ? "px-2" : "px-3"}`}>
                    <Link
                        ...
                        className={`flex items-center gap-3 rounded-md py-2.5 
                        text-sm text-admin-sidebar-fg/80 hover:bg-white/5 
                        hover:text-white ${
                    ...
                    <button
                        ...
                        aria-label="Logout"
                        className={`flex w-full items-center gap-3 rounded-md py-2.5 
                        text-sm text-admin-sidebar-fg/80 hover:bg-white/5 
                        hover:text-white ${
                            collapsed ? "justify-center px-2" : "px-3"
                        }`}
